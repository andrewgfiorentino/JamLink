...
That split has a trap on the reporting side, and the connection grade has been
caught by it once already. When a stream stops arriving, playout parks past the
last packet: depth reads zero, and every pull takes the stretch branch, so
nothing is booked as concealment. Graded on those two numbers alone, a silent
link scores *better* than a working one. `AppController::connectionQuality`
therefore checks for the absence of arrivals first and says so outright.

Three rules for anything that reports connection health:

- Divide a stream's loss by that stream's own accepted count. The transport-wide
  `packetsReceived` includes the other stream and all control traffic, which
  understates loss by roughly half.
- Measure over a trailing window, not the session. Lifetime totals go deaf: an
  hour of clean play makes the denominator so large that a fresh dropout cannot
  move it.
- Zero round trip means "no pong yet", not "0 ms". `roundTripMeasured` carries
  that distinction, because the UI uses the word "measured".
